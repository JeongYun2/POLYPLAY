...
function back(){
 ...
 function check(){	
	...
	if(formname.cSubject.value == ""){
		...
		if (res == true){
			formname.action ="${pageContext.request.contextPath}/AdminContentsModifyAction";
...
<table>
			...
			<td  colspan = '4'> 
			<select name = "gidx">
			<option value ="1" <c:if test="${cv.gidx ==1}">selected</c:if>>액션</option>
			<option value ="2" <c:if test="${cv.gidx ==2}">selected</c:if>>스릴러</option>
			<option value ="3" <c:if test="${cv.gidx ==3}">selected</c:if>>SF판타지</option>
			<option value ="4" <c:if test="${cv.gidx ==4}">selected</c:if>>공포</option>
			<option value ="5" <c:if test="${cv.gidx ==5}">selected</c:if>>코미디</option>
			<option value ="6" <c:if test="${cv.gidx ==6}">selected</c:if>>로맨스</option>
			<option value ="7" <c:if test="${cv.gidx ==7}">selected</c:if>>드라마</option>
			<option value ="8" <c:if test="${cv.gidx ==8}">selected</c:if>>애니메이션</option>
			</select>
			</td>
			</tr>
			...
			<tr>
				<td colspan='1'>작품명</td>
				<td colspan='4'><input type="text" name="cSubject" value="${cv.cSubject}"
					size="20" maxlength="20" /></td>
			</tr>
			<tr>
				<td colspan='1'>개봉연도</td>
				<td colspan='4'><input type="text" name="cOpenYear"
					value="${cv.cOpenYear}" size="20" maxlength="20" /></td>
			</tr>
			<tr>
				<td colspan='1'>러닝타임</td>
				<td colspan='4'><input type="text" name="cRuntime" value="${cv.cRuntime}"
					size="20" maxlength="30"></td>
			</tr>
			<tr>
				<td colspan='1'>감독</td>
				<td colspan='4'><input type="text" name="cDirector" value="${cv.cDirector}"
					size="20" maxlength="20" /></td>
			</tr>
			<tr>
				<td colspan='1'>금액</td>
				<td colspan='4'><input type="text" name="cPrice" value="${cv.cPrice}"
					size="20" maxlength="20" /></td>
			</tr>
			<tr>
				<td colspan='1'>배우</td>
				<td colspan='4'><input type="text" name="cActor" value="${cv.cActor}"
					size="20" maxlength="20" /></td>
			</tr>
			<tr>
			<td>줄거리</td>
			...
			<td colspan ="5"><textarea name="cStory" cols="200" rows="10">${cv.cStory}</textarea>
				</td>
			</tr>
